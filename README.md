...
- 메모 작성, 다운로드, 공유하기 등이 주요 기능입니다. 이러한 기능 구현에 `contentEditable`을 사용.

- 소켓을 통해 다른 사용자와 함께 메모할 수 있도록 하였고 이때, 사용자의 커서 위치를 시각화.

- Google Cloud Vision API를 사용하여 작성한 수기로 작성한 메모에서 글씨를 인식하여 데이터화. (OCR)
...
- 메모 앱 애플리케이션을 구현하기 위해선 사용자의 입력을 전달받아야 합니다. 이를 위해서 처음에는 `textarea`를 사용하여 개발을 시작했습니다. `textarea`를 사용하면 별도의 추가 조치 없이 React로 사용자 입력을 렌더링할 수 있습니다. 하지만 사용자의 커서 위치를 시각화하기 위해 좌표를 계산하기 위해 `textarea`를 사용하기에 적합하지 않다고 판단했습니다.
...
- **상세 설명**
  ...
  | **5. 타겟 노드 좌표 계산**      | 상대방 사용자가 마지막으로 입력한 DOM 에서 `targetNode`의 위치를 `getBoundingClientRect()` 메소드를 사용하여 계산합니다.                                                                                                                                     |
